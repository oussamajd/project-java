...
/**
 * Created by TekSliveron 8/3/2018.
 */
public class Student {
    ...
    private void setStudentEditProfileClick(Event event){
        setAllEnable();
        String sqlQuery = "select * FROM student where dbStudentID = '" + ID +"';";//add_query
        try {
            connection = database.getConnection();
            statement = connection.createStatement();
    ...
    private void setStudentRegistrationClick(Event event) throws IOException {
        ...
        FXMLloader.setLocation(getClass().getResource("/student/Registration.fxml"));
        
        FXMLloader.load();
        Parent p = (Parent)FXMLloader.getRoot();
        Stage stage = new Stage();
        stage.setScene(new Scene(p));
