...
public class ProfileView {
    ...
    public void setCurrentInfo(Info currentInfo) {
       ...
       //add code
        ...
        tDepartment.setLayoutX(tName.getLayoutX() + 10.0 + tName.getBoundsInParent().getWidth());
    ...
    private void setLoadGpaButtonClcik(Event event){
        ...
        XYChart.Series<String,Number> gpaLineChart = new Series<String, Number>();
        // add code
        try
        {
          ...
          {
            for (int i = 1; i < 12; i++) {
              if (resultSet.getDouble("dbStudent" + i + "thSemGpa") != 0.0D) {
                gpaLineChart.getData().add(new XYChart.Data<String,Number>(i + "th", Double.valueOf(resultSet.getDouble("dbStudent" + i + "thSemGpa"))));
              }
            }
